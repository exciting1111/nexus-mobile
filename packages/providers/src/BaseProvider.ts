...
export abstract class BaseProvider extends SafeEventEmitter {
   ...
   * If null, Rabby is either locked or the user has not permitted any
   ...
   * @param initialState.isUnlocked - Whether the user has unlocked Rabby.
   ...
   * @fires RabbyInpageProvider#connect
   */
  ...
  protected _handleAccountsChanged(
    ...
    if (!Array.isArray(accounts)) {
      this._log.error(
        'Rabby: Received invalid accounts parameter. Please report this bug.',
        accounts,
      );
      _accounts = [];
    }
    ...
    for (const account of accounts) {
      if (typeof account !== 'string') {
        this._log.error(
          'Rabby: Received non-string account. Please report this bug.',
          accounts,
        );
        _accounts = [];
        break;
    ...
    if (!dequal(this._state.accounts, _accounts)) {
      ...
      if (isEthAccounts && this._state.accounts !== null) {
        this._log.error(
          `Rabby: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`,
          _accounts,
        );
      }
  ...
  protected _handleUnlockStateChanged({
    ...
    if (typeof isUnlocked !== 'boolean') {
      this._log.error(
        'Rabby: Received invalid isUnlocked parameter. Please report this bug.',
      );
      return;
    }
