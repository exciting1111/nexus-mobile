...
const tokenTableName = `${APP_DB_PREFIX}cache_balance`;
...
export class UpdateBalanceAddEvmUsdValue1751964116411
  implements MigrationInterface
{
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (tableExists) {
      ...
      await queryRunner.query(
        `ALTER TABLE '${tokenTableName}' ADD COLUMN evm_usd_value REAL DEFAULT 0`,
      );
    }
  }
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (tableExists) {
      await queryRunner.query(
        `ALTER TABLE '${tokenTableName}' DROP COLUMN evm_usd_value`,
      );
    }
  }
}
