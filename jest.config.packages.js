...
module.exports = {
  ...
  // Here we ensure that Jest resolves `@rabby-wallet/*` imports to the uncompiled source code for packages that live in this repo.
  // NOTE: This must be synchronized with the `paths` option in `tsconfig.packages.json`.
  moduleNameMapper: {
    '^@rabby-wallet/(.+)$': [
      '<rootDir>/../$1/src',
      // Some @rabby-wallet/* packages we are referencing aren't in this monorepo,
      // so in that case use their published versions
      '<rootDir>/../../node_modules/@rabby-wallet/$1',
    ],
  },
